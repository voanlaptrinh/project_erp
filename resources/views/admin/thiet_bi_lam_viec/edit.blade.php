...
@section('body')
    ...
    {{-- <div class="container">
        ...
        <form method="POST" action="{{ route('thietbi.update', $device->id) }}">
            @csrf
            @method('PUT')
...
<div class="mb-3">
    ...
    <input type="text" class="form-control" name="loai_thiet_bi" value="{{ old('loai_thiet_bi', $device->loai_thiet_bi ?? '') }}" required>
</div>

<div class="mb-3">
    <label for="ten_thiet_bi" class="form-label">Tên thiết bị</label>
    <input type="text" class="form-control" name="ten_thiet_bi" value="{{ old('ten_thiet_bi', $device->ten_thiet_bi ?? '') }}">
</div>

<div class="mb-3">
    <label for="he_dieu_hanh" class="form-label">Hệ điều hành</label>
    <input type="text" class="form-control" name="he_dieu_hanh" value="{{ old('he_dieu_hanh', $device->he_dieu_hanh ?? '') }}">
</div>
...
<div class="mb-3">
    ...
    <input type="text" class="form-control" name="so_serial" value="{{ old('so_serial', $device->so_serial ?? '') }}">
</div>

<div class="mb-3">
    <label for="ngay_ban_giao" class="form-label">Ngày bàn giao</label>
    <input type="date" class="form-control" name="ngay_ban_giao" value="{{ old('ngay_ban_giao', $device->ngay_ban_giao ?? '') }}">
</div>

<div class="mb-3">
    <label for="ghi_chu" class="form-label">Ghi chú</label>
    <textarea class="form-control" name="ghi_chu">{{ old('ghi_chu', $device->ghi_chu ?? '') }}</textarea>
</div>

            
            <button type="submit" class="btn btn-primary">Cập nhật</button>
            <a href="{{ route('thietbi.index') }}" class="btn btn-secondary">Hủy</a>
        </form>
    </div> --}}
